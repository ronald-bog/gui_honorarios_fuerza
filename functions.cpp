...
// Funci�n para dividir una cadena en un vector de cadenas usando un delimitador
...
// Funci�n para unir un vector de cadenas en una sola cadena usando un delimitador
...
void functions(int colFileA, int newColumFileA, const string& fileB, int colFindB, int colResultB) {
    string fileA = "result.csv"; // Archivo principal (se modificar� directamente)

    // Mapa para almacenar los valores de b�squeda y copia del fileB
    ...
    while (getline(br, linea)) {
        ...
        if (valores.size() > colResultB) {
            string clave = valores[colFindB]; // Valor de b�squeda
    ...
    // Leer el fileA y almacenar las l�neas en una lista
    ...
    // Arreglo para almacenar las l�neas procesadas en el orden original
    vector<string> lineasProcesadas(lineas.size());

    // N�mero de hilos (usamos el n�mero de n�cleos disponibles)
    unsigned int numHilos = thread::hardware_concurrency();
    vector<future<void>> futures;

    // Dividir las l�neas en bloques para procesamiento concurrente
    int numLineas = lineas.size();
    int tamanoBloque = (numLineas + numHilos - 1) / numHilos; // Tama�o de cada bloque

    for (unsigned int i = 0; i < numHilos; i++) {
        int inicio = i * tamanoBloque;
        int fin = min(inicio + tamanoBloque, numLineas);

        // Verificar que el �ndice de inicio no sea mayor que el n�mero de l�neas
        if (inicio >= numLineas) {
            break; // Salir del bucle si no hay m�s l�neas para procesar
        }

        // Crear una tarea para procesar el bloque de l�neas
        futures.push_back(async(launch::async, [&, inicio, fin]() {
            for (int j = inicio; j < fin; j++) {
                string lin = lineas[j];
                vector<string> valores = split(lin, ';');
                if (valores.size() > newColumFileA) {
                    string clave = valores[colFileA]; // Valor de b�squeda en fileA
                    // Si hay coincidencia, copiar el valor al fileA
                    valores[newColumFileA] = mapaBusqueda.count(clave) ? mapaBusqueda[clave] : "errorXX";
                }
                // Reconstruir la l�nea actualizada y guardarla en la posici�n correcta
